...
def rdd():
    message = Message(
        ...
        fields=[
            ...
            Field(name="string", value_type=Field.STRING, value_string=["foo"]),
            Field(name="bool", value_type=Field.BOOL, value_bool=[True]),
            Field(name="double", value_type=Field.DOUBLE, value_double=[4.2]),
            Field(name="integer", value_type=Field.INTEGER, value_integer=[42]),
            Field(name="string-with-int-value", value_type=Field.STRING, value_string=["42"]),
            ...
            """]),
            Field(name="extracted.subfield", value_type=Field.STRING, value_string=['{"delta": "4"}']),
            Field(name="extracted.nested.subfield", value_type=Field.STRING, value_string=['{"epsilon": "5"}']),
            Field(name="partiallyExtracted.nested", value_type=Field.STRING, value_string=['{"zeta": "6"}']),
        ]
    ).SerializeToString()
    ...
    with mock_s3():
        ...
        for item in s3.get_paginator("list_objects").paginate(Bucket=bucket).build_full_result()["Contents"]:
